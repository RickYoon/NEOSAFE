const { CONST, api,tx,wallet, rpc,sc, logging, u } = require("@cityofzion/neon-js")


async function invokeContract (baseAddress, userAccount, chainProvider) {

  const request = {
    "id": 1694934220143784,
    "topic": "554e5a99fe9063a52a3265fc657372ba627bfe07bf068557db7a59c64e76d056",
    "params": {
        "request": {
            "id": 1,
            "jsonrpc": "2.0",
  ...
  if(chainProvider === "NEO-N3-TEST"){
    providerUrl = "https://testnet2.neo.coz.io/"
  } else {

  }

  const rpcClient = new rpc.RPCClient(providerUrl);

  // 1. Sender 지갑 정보 받아오기 (base wallet 선택했을 때 처리방안 추가 필요.)
  const walletOnboard = await updateWalletInfo(baseAddress, userAccount, chainProvider)
  console.log("walletOnboard",walletOnboard)

  const operation = request.params.request.params.invocations[0].operation
  const args = request.params.request.params.invocations[0].args
...
async function updateWalletInfo (baseAddress, userAccount, chainProvider) {

  const url = `https://blqkysq4r3.execute-api.ap-northeast-2.amazonaws.com/production/walletlist?address=${baseAddress}&chain=${chainProvider}`;

  const walletData = await fetch(url)
  .then((response) => {
    if (!response.ok) {
  ...
  } else { // multi-sig wallet

      // console.log("walletData",walletData.Items[0].smartWallets[0])
      walletInfo = walletData.Items[0].smartWallets[0]
  }
...
async function createTransaction(rpcClient,inputs, vars) {
  ...
  const extractedArgs = extractValuesFromArray(inputs.args);
...
// sendNeoToken()
module.exports = {
  invokeContract, // 필요한 다른 함수도 필요에 따라 내보낼 수 있습니다.
};
